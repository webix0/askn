...
function wprig_fonts_url() {
	...
	/**
	 * Translator: If Josefin Sans does not support characters in your language, translate this to 'off'.
	 */
	$josefin = esc_html_x( 'on', 'Josefin Condensed font: on or off', 'wprig' );
	/**
	...
	if ( 'off' !== $josefin ) {
		$font_families[] = 'Josefin+Sans:100i,300,300i,400,400i,700';
	}

	if ( 'off' !== $lato ) {
		$font_families[] = 'Lato:300,400,400i,700i,900';
	}

	if ( in_array( 'on', array( $josefin, $lato ) ) ) {
		$query_args = array(
